...
const collectKeyValuePairs = (itemData: Record<string, any>, keyValueMap: Record<string, Set<string>>) => {
  for (const [key, value] of Object.entries(itemData)) {
    ...
    if (typeof value === "string" || typeof value === "number") {
      ...
      value.forEach((item) => keyValueMap[key].add(typeof item === "object" ? JSON.stringify(item) : String(item)));
...
export const fetchAndLogEndpointData = async (selectedEndpoint: keyof typeof endpoints) => {
  ...
  try {
    ...
    if (typeof data === "object" && data !== null && "results" in data && Array.isArray(data.results)) {
      ...
      console.log(`  Response data from ${selectedEndpoint} is not in expected format:`, data);
    }
  } catch (error) {
    console.error(`Error fetching data for endpoint ${selectedEndpoint}:`, error);
  }
};
